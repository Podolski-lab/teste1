...
**Resposta direta:** uma versão *básica* (lembretes fixos ligados ao calendário) é viável hoje sem código e sem custo. A visão completa do produto — lembretes dinâmicos por pilar e checkpoints verbais — não é.
...
Existe integração nativa e gratuita (app Alexa → Configurações → Calendário): leitura/escrita básica de eventos, avisos pré-compromisso. Recurso padrão da Alexa (não exclusivo do Alexa+). Limitação: sem tags/categorização por pilar, sem puxar dinamicamente uma lista de tarefas, sem sincronização instantânea.
...
Lançado em 2025, EUA/Canadá apenas (até set/2026), grátis com Prime ou US$19,99/mês. Adiciona fluência conversacional e algum comportamento proativo (ex.: ingerir agenda de e-mails, sugerir horários considerando trânsito, gerar alertas contextuais via "Omnisense"). Tem "Actions" agênticas (Web Action SDK), mas voltadas para tarefas comerciais (reservas, compras) — não há recurso documentado para diálogos de accountability personalizados.

### Skills de terceiros existentes
A categoria "Calendars & Reminders" da loja de Skills tem conectores genéricos (Google Calendar, Todoist, Any.do), mas nenhuma Skill identificada oferece coaching de accountability por pilares com checkpoints verbais — nicho não preenchido.
...
**Ressalva:** Alexa+ é um rollout em rápida evolução (2025-2026); os recursos agênticos ainda são majoritariamente voltados a comércio.
